...
data_trainval = Dataset("inviscid_burgers/inviscid_burgers_dataset.h5", res=res)
data_test     = Dataset("inviscid_burgers/inviscid_burgers_dataset.h5", res=res_eval)
...
if train:
    ...
    torch.save(model.state_dict(), f"inviscid_burgers/{filename}.pt")
    print(f"✅ Trained model saved to 'inviscid_burgers/{filename}.pt'")

if not train:
    model.load_state_dict(torch.load(f"inviscid_burgers/{filename}.pt", map_location=device, weights_only=False))


# %% 5. Model Evaluation ----------------------------------------------------------------------------------------
if eval:
    evaluate_model_1d(model, test_loader, device=device, visualise=True, data_processor=data_processor,
                      title='FNO for 1D Inviscid Burgers', filename='inviscid_burgers_io.png', n_vis_samples=3)