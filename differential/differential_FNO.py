...
model_summary(verbose=verbose, title="#### FNO 2D-DIFFERENTIAL ####",
...
if train:
    ...
    torch.save(model.state_dict(), f"differential/{filename}.pt")
    print(f"✅ Trained model saved to 'differential/{filename}.pt'")

if not train:
    model.load_state_dict(torch.load(f"differential/{filename}.pt", map_location=device, weights_only=False))

# %% 5. Model Evaluation ----------------------------------------------------------------------------------------
if eval:
    evaluate_model_2d(model, test_loader, device=device, visualise=True, data_processor=data_processor,
                      title='FNO for 2D Differential', filename='differential_io.png')
