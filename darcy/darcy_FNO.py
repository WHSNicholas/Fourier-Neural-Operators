...
#                                           2D Darcy Equation - FNO Training                                           #
# -------------------------------------------------------------------------------------------------------------------- #
...
model_summary(verbose=verbose, title="#### FNO 2D-DARCY ####",
...
if train:
    ...
    torch.save(model.state_dict(), f"darcy/{filename}.pt")
    print(f"✅ Trained model saved to 'darcy/{filename}.pt'")

if not train:
    model.load_state_dict(torch.load(f"darcy/{filename}.pt", map_location=device, weights_only=False))
...
if eval:
    evaluate_model_2d(model, test_loader, device=device, visualise=True, data_processor=data_processor,
                      title='FNO Model on 2D Darcy', filename='darcy_io.png')