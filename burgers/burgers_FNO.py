...
res_eval = 2**12                           # Evaluation Resolution
filename = f"burgers_model_{res}_large"   # Filename
...
hidden_channels = 32                      # Dimension of Latent Representation
...
data_trainval = Dataset("burgers/burgers_dataset.h5", res=res)
data_test     = Dataset("burgers/burgers_dataset.h5", res=res_eval)
...
model_summary(verbose=verbose, title="#### FNO 1D-BURGERS ####",
...
if train:
    ...
    torch.save(model.state_dict(), f"burgers/{filename}.pt")
    print(f"✅ Trained model saved to 'burgers/{filename}.pt'")

if not train:
    model.load_state_dict(torch.load(f"burgers/{filename}.pt", map_location=device, weights_only=False))


# %% 5. Model Evaluation ----------------------------------------------------------------------------------------
if eval:
    evaluate_model_1d(model, test_loader, device=device, visualise=True, data_processor=data_processor,
                      title='FNO for 1D Burgers', filename='burgers_io.png', n_vis_samples=3)